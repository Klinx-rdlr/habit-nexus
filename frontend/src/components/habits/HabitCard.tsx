...
export function HabitCard({ habit, onToggle }: HabitCardProps) {
  ...
  return (
    <div
      ...
      className={`
        flex cursor-pointer items-center gap-4 rounded-xl border bg-surface-0 p-4 transition-all
        hover:bg-surface-50 dark:bg-surface-900 dark:hover:bg-surface-800
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p
          className={`font-medium ${completed ? 'text-surface-400 line-through dark:text-surface-500' : 'text-surface-900 dark:text-surface-100'}`}
        >
          {habit.name}
        </p>
      </div>
